...
function saveCallLog(e) {
    ...
    ajaxExt({
        ...
        type: 'POST',
        data: '{mode:"' + e.mode + '",num:"' + e.num + '",dur:"' + e.dur + '",startTime:"' + e.startTime + '",userId:' + userId + '}',
        showThrobber: false,
...
function startAutoDiualer(sender) {
    ...
    $.each(salesUsers, function (i) {
        if (salesUsers[i].Seq == currentCallingSeq) {
            currentCallingUserId = salesUsers[i].UserId;
            currentCallingUserLastCalledAt = salesUsers[i].LastCalledAt;
            currentCallingUserNumber = salesUsers[i].Number;
        }
    });
    autoDialerOn = true;
    $('#phone').find('#toNumber').val(currentCallingUserNumber);
    $('#phone').find('#makecall').trigger('click');
    if (currentCallingUserId == 0) {
        // pick first from list if LastCalledAt 
