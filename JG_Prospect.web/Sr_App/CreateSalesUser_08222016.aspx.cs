...
namespace JG_Prospect.Sr_App
{
    public partial class CreateSalesUser : System.Web.UI.Page
    {
        ...
        protected void btnSaveInterview_Click(object sender, EventArgs e)
        {
            ...
            DataSet ds = InstallUserBLL.Instance.ChangeStatus(Convert.ToString(ddlstatus.SelectedValue), EditId, interviewDate.ToString("yyyy-MM-dd"), ddlInsteviewtime.SelectedItem.Text, Convert.ToInt32(Session[JG_Prospect.Common.SessionKey.Key.UserId.ToString()]), string.Empty);
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
        ...
        private void LoadUsersByRecruiterDesgination()
        {
            ...
            dsUsers = TaskGeneratorBLL.Instance.GetInstallUsers(2, "Recruiter");
