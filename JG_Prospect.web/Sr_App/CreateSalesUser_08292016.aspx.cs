...
namespace JG_Prospect.Sr_App
{
    public partial class CreateSalesUser : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["ID"] != null)
                {
                    ...
                    if (ds.Tables[0].Rows.Count != 0)
                    {
                        ...
                        //Yogesh Keraliya :
                        //Issue solved: every users designation was getting change to Jr.Sales when it was edited and saved.
        ...
        protected void btnSaveInterview_Click(object sender, EventArgs e)
        {
            ...
            DataSet ds = InstallUserBLL.Instance.ChangeStatus(Convert.ToString(ddlstatus.SelectedValue), EditId, interviewDate.ToString("yyyy-MM-dd"), ddlInsteviewtime.SelectedItem.Text, Convert.ToInt32(Session[JG_Prospect.Common.SessionKey.Key.UserId.ToString()]), string.Empty, ddlUsers.SelectedValue);
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
            ...
            SendEmail(email, txtfirstname.Text, txtlastname.Text, "Interview Date Auto Email", string.Empty, string.Empty, HireDate, EmpType, PayRates, 104);
