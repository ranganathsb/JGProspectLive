    ...
    public partial class TaskGenerator : System.Web.UI.UserControl
    {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (this.IsAdminMode)
                {
                    ...
                    if (
                        ...
                        string.IsNullOrEmpty(Convert.ToString(DataBinder.Eval(e.Row.DataItem, "FristName")))
                       )
                    {
                        ...
                        DataSet dsUsers = TaskGeneratorBLL.Instance.GetInstallUsers(2, Convert.ToString(DataBinder.Eval(e.Row.DataItem, "Designation")).Trim());
                    ...
                    else if (ddlgvTaskStatus.SelectedValue == Convert.ToByte(TaskStatus.Requested).ToString())
                    {
                        ...
                        hypUsers.Visible = false;
                        String[] requestVal = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "RequestFristName")).Split(':');

                        if (requestVal.Length > 1)
                        {
                            lbtnRequestStatus.Text = requestVal[1]; 
                        }

                        lbtnRequestStatus.ForeColor = System.Drawing.Color.Red;
                        lbtnRequestStatus.CommandName = "approve-request";
                        lbtnRequestStatus.CommandArgument = DataBinder.Eval(e.Row.DataItem, "TaskId").ToString() + ":" + Convert.ToString(DataBinder.Eval(e.Row.DataItem, "RequestFristName")).Split(':')[0];
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (ddlgvTaskStatus.SelectedValue == Convert.ToByte(TaskStatus.Open).ToString() &&
                        strMyDesignation == Convert.ToString(DataBinder.Eval(e.Row.DataItem, "Designation")).Trim().ToLower())
                    {
                        lbtnRequestStatus.Visible = true;
                        ddcbAssigned.Visible =
                        hypUsers.Visible = false;
        ...
        private void SendEmailToAssignedUsers(string strInstallUserIDs)
        {
            try
            {
                ...
                int EmailTemplateID = 108;
                DataSet dsEmailTemplate = AdminBLL.Instance.GetEmailTemplate(strHTMLTemplateName,EmailTemplateID);
