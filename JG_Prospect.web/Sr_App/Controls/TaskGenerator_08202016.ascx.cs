            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (this.IsAdminMode)
                {
                    ...
                    else if (ddlgvTaskStatus.SelectedValue == Convert.ToByte(TaskStatus.Requested).ToString())
                    {
                        ...
                        hypUsers.Visible = false;
                        string[] requeststatus = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "RequestFristName")).Split(':');

                        lbtnRequestStatus.Text = requeststatus.Length > 1 ? requeststatus[1]: requeststatus[0];
        ...
        private void SendEmailToAdminUser(int intTaskCreatedBy)
        {
            try
            {
                if (intTaskCreatedBy > 0)
                {
                    ...
                    DataSet dsEmailTemplate = AdminBLL.Instance.GetEmailTemplate(strHTMLTemplateName);
                    DataSet dsUser = TaskGeneratorBLL.Instance.GetInstallUserDetails(this.TaskCreatedBy);
